...
for _ in result_bi:
    ...
    for __ in range(len(for_result)):
        if for_result[__] == '*':
            for_result[__ + 1] = for_result[__ - 1] * for_result[__ + 1]
            for_result[__] = -0xfffffffffffffffff
            for_result[__ - 1] = -0xfffffffffffffffff
    for_real_result = []
    for __ in for_result:
        if __ == -0xfffffffffffffffff:
            continue
        else:
            for_real_result.append(__)
    result = for_real_result[0]
    for __ in range(len(for_real_result)):
        if for_real_result[__] == '+':
            result += for_real_result[__ + 1]
        if for_real_result[__] == '-':
            result -= for_real_result[__ + 1]
        if for_real_result[__] == '*':
            result *= for_real_result[__ + 1]
